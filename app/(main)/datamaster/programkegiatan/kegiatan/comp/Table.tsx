...
import { ModalMasterKegiatan } from "./ModalMasterKegiatan";

const Table = () => {

    const [Data, setData] = useState<GetResponseFindallKegiatan[]>([]);
    const [ModalOpen, setModalOpen] = useState<boolean>(false);
    const [JenisModal, setJenisModal] = useState<"tambah" | "edit">("tambah");
    const [DataModal, setDataModal] = useState<GetResponseFindallKegiatan | null>(null);
    ...
    const handleModal = (jenis: "tambah" | "edit", data: GetResponseFindallKegiatan | null) => {
        if (ModalOpen) {
            setModalOpen(false);
            setDataModal(null);
    ...
    useEffect(() => {
        const getBidangUrusan = async () => {
            setLoading(true);
            await apiFetch<any>(`${branding?.api_perencanaan}/kegiatan/findall`, {
                method: "GET",
            }).then((resp) => {
    ...
    const hapusData = async (id: any) => {
        setProses(true);
        await apiFetch(`${branding?.api_perencanaan}/kegiatan/delete/${id}`, {
            method: "DELETE",
        }).then(resp => {
    ...
    if (Loading) {
        ...
        return (
            <>
                <Card>
                    <HeaderCard>
                        <h1 className="font-bold text-lg uppercase">Master Kegiatan</h1>
                        <ButtonSky
                    ...
                    <div className="flex flex-wrap m-2">
                        <div className="overflow-auto m-2 rounded-t-xl border border-green-700 w-full">
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-green-700 text-white">
                                        <th className="border-r border-b border-gray-200 px-6 py-3 w-[50px] text-center">No</th>
                                        <th className="border-r border-b border-gray-200 px-6 py-3 min-w-[200px]">Nama Kegiatan</th>
                                ...
                                <tbody>
                                    {Data.length > 0 ?
                                        Data.map((item: GetResponseFindallKegiatan, index: number) => (
                                            <tr key={index}>
                                                <td className="border-r border-b border-green-700 py-4 px-3 text-center">{index + 1}</td>
                                                <td className="border-r border-b border-green-700 px-6 py-4 text-center">{item.nama_kegiatan || "-"}</td>
                                                <td className="border-r border-b border-green-700 px-6 py-4 text-center">{item.kode_kegiatan || 0}</td>
                                                <td className="border-b border-green-700 px-6 py-4 text-center">
