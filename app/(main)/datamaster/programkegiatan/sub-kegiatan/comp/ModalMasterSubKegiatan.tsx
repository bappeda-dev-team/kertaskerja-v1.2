...
interface modal {
    ...
    onClose: () => void;
    onSuccess: () => void;
    Data: GetResponseFindallSubKegiatan | null
}
interface FormValue {
    id: string;
...
export const ModalMasterSubKegiatan: React.FC<modal> = ({ isOpen, onClose, onSuccess, jenis, Data }) => {
    ...
    const onSubmit: SubmitHandler<FormValue> = async (data) => {
        ...
        try {
            setProses(true);
            await apiFetch(jenis === "tambah" ? `${branding?.api_perencanaan}/sub_kegiatan/create` : `${branding?.api_perencanaan}/sub_kegiatan/update/${Data?.id}`, {
                method: jenis === "tambah" ? "POST" : "PUT",
                body: formData as any
            }).then(_ => {
                AlertNotification("Berhasil", "Berhasil Menyimpan Data Sub Kegiatan", "success", 3000, true);
                onSuccess();
                handleClose();
            }).catch(err => {
                ...
                <div className={`bg-white rounded-lg p-8 z-10 w-4/5 max-h-[80%] text-start overflow-auto`}>
                    ...
                    >
                        <div className="border-b">
                            <h1 className="text-xl font-semibold uppercase text-center">{jenis} Sub Kegiatan</h1>
                        </div>
                        <div className="flex flex-col py-3">
                            <label className="uppercase text-xs font-bold text-gray-700 my-2">
                                Nama Sub Kegiatan : {errors.nama_subkegiatan && "wajib diisi"}
                            </label>
                            <Controller
                        ...
                        <div className="flex flex-col py-3">
                            <label className="uppercase text-xs font-bold text-gray-700 my-2">
                                Kode Sub Kegiatan : {errors.kode_subkegiatan && "wajib diisi"}
                            </label>
                            <Controller
