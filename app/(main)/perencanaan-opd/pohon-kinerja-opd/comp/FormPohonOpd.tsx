...
export const FormPohonOpd: React.FC<{
    ...
    fetchTrigger: () => void;
    deleteTrigger: () => void;
}> = ({ id, level, formId, onCancel, fetchTrigger, deleteTrigger }) => {
    ...
    const onSubmit: SubmitHandler<FormValue> = async (data) => {
        ...
        try {
            setProses(true);
            let url = `${branding?.api_perencanaan}/pohon_kinerja_opd/create`;
            setProses(true);
            await apiFetch(url, {
                method: "POST",
                        ...
                        <div className="flex justify-center my-3 w-full">
                            ...
                            >
                                ...
                                <label className="uppercase text-base font-bold text-sky-700 my-2">
                                    Indikator {LabelJenisPohon("tambah", level)}
                                </label>
                                ...
                                <ButtonRed className="flex items-center gap-1  w-full my-3" onClick={onCancel} disabled={Proses}>
                                    <TbCircleX />
                                    Batal
                                </ButtonRed>
                            </form>
...
export const FormEditPohon: React.FC<{
...
}> = ({ id, level, EditBerhasil, onCancel }) => {
    ...
    const onSubmit: SubmitHandler<FormValue> = async (data) => {
        ...
        // console.log(formData);
        setProses(true);
        let url = `${branding?.api_perencanaan}/pohon_kinerja_opd/update/${id}`;
        setProses(true);
        await apiFetch(url, {
            method: "PUT",
            body: JSON.stringify(formData)
            ...
            if (resp.code === 200 || resp.code === 201) {
                ...
                AlertNotification("Gagal", `${resp.data}`, "error", 2000);
            }
        }).catch(err => {
            AlertNotification("Gagal", `${err}`, "error", 3000, true);
        }).finally(() => {
    ...
    return (
        <>
            <div
                className={`tf-nc tf flex flex-col w-[600px] rounded-lg shadow-lg form-pohon
                    ${shadowJenisPohon("edit", level)}
                `}
            >
                <div className="flex pt-3 justify-center font-bold text-lg uppercase border my-3 py-3 border-black rounded-lg">
                    Edit {LabelJenisPohon("edit", level)}
                </div>
                <div className="flex justify-center my-3 w-full">
                    <form
                        onSubmit={handleSubmit(onSubmit)}
                    ...
                    >
                        ...
                        <label className="uppercase text-base font-bold text-sky-700 my-2">
                            Indikator {LabelJenisPohon("edit", level)}
                        </label>
                        {fields.map((field, index) => (
                            <div key={index} className="flex flex-col my-2 py-2 px-5 border border-sky-700 rounded-lg">
                                <Controller
