...
import { ButtonSky, ButtonSkyBorder, ButtonRedBorder, ButtonGreenBorder, ButtonBlackBorder } from '@/components/ui/button';
import { AlertNotification, AlertQuestion } from '@/lib/alert';
import { FormPohonOpd, FormEditPohon } from './FormPohonOpd';
import { ModalAddCrosscutting } from './ModalCrosscutting';
import { ModalPindahPohonOpd } from './ModalPindahPohon';
...
export const PohonOpd: React.FC<pohon> = ({ tema, deleteTrigger, fetchTrigger, show_all, show_detail, set_show_all }) => {
    ...
    const fetchPohonCross = async (id: string) => {
        ...
        await apiFetch(`${branding?.api_perencanaan}/crosscutting_opd/findall/${id}`, {
            method: "GET"
        }).then((result: any) => {
    ...
    const hapusPohonOpd = async (id: any) => {
        await apiFetch(`${branding?.api_perencanaan}/pohon_kinerja_opd/delete/${id}`, {
            method: "DELETE"
        }).then((result: any) => {
            if (result.code === 200) {
    ...
    const hapusPohonCross = async (id: any) => {
        await apiFetch(`${branding?.api_perencanaan}/crosscutting_opd/delete/${id}/${branding?.user?.nip}`, {
            method: "DELETE"
        }).then((result: any) => {
            if (result.code == 400) {
                            ...
                            >
                                ...
                                <div
                                    className={`flex pt-3 justify-center font-bold text-lg uppercase border my-3 py-3 rounded-lg bg-white
                                            ${tema.jenis_pohon === "Strategic Pemda" && 'border-red-700 text-white bg-linear-to-r from-[#CA3636] from-40% to-[#BD04A1]'}
                                            ${tema.jenis_pohon === "Tactical Pemda" && 'border-blue-500 text-white bg-linear-to-r from-[#3673CA] from-40% to-[#08D2FB]'}
                                            ${tema.jenis_pohon === "Operational Pemda" && 'border-green-500 text-white bg-linear-to-r from-[#007982] from-40% to-[#2DCB06]'}
                                ...
                                {(branding?.user?.roles == 'super_admin' || branding?.user?.roles == 'admin_opd' || branding?.user?.roles == 'level_1') &&
                                ...
                                <div className="flex justify-evenly my-3 py-3 hide-on-capture">
                                    {(tema.level_pohon != 4 && (
                                        ...
                                        branding?.user?.roles == 'level_1' ||
                                        branding?.user?.roles == 'level_2'
                                    )) &&
                                        <>
                                            <ButtonBlackBorder className={`px-3 bg-white flex justify-center items-center py-1 bg-linear-to-r rounded-lg`}
                                    ...
                                    {(branding?.user?.roles == 'super_admin' || branding?.user?.roles == 'admin_opd' || branding?.user?.roles == 'level_1' || branding?.user?.roles == 'level_2') &&
                                        Show &&
                                        <ButtonGreenBorder className={`px-3 bg-white flex justify-center items-center py-1 bg-linear-to-r rounded-lg`}
                                            onClick={newChild}
                                        >
...
export const TablePohon = (props: any) => {
    ...
    useEffect(() => {
        const fetchOpdAsal = async () => {
            setProses(true);
            await apiFetch(`${branding?.api_perencanaan}/crosscutting_opd/opd-from/${id}`, {
                method: "GET"
            }).then((result: any) => {
                if (result.code === 500) {
                    setOpdAsal(null);
                } else {
                    setOpdAsal(result.data.nama_opd);
                }
            }).catch((err) => {
    ...
    return (
        <div className='flex flex-col w-full'>
            ...
            <div className="flex mt-2 w-full justify-center bg-white rounded-lg hide-on-capture">
                <ButtonBlackBorder className='w-full' type="button" onClick={() => setShow((prev) => !prev)}>
                    {Show ?
                        <div className="flex items-center gap-1">
                            <TbEye />
                            Sembunyikan
            ...
            <div
                className={`flex justify-evenly border my-3 py-3 rounded-lg bg-white hide-on-capture
                        ${jenis === "Operational N" && 'border-green-500'}
                        `}
            >
                ...
                <button
                    className={`px-3 flex justify-center items-center py-1 rounded-lg border border-red-400 text-red-400 hover:bg-red-400 hover:text-white cursor-pointer ${review > 0 && "bg-yellow-200 animate-pulse"}`}
                    type="button"
                    onClick={() => {
